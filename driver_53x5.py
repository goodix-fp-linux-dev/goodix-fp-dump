...
VALID_FIRMWARE = r"GF5288_HTSEC_APP_100(11|20)"
...
def is_valid_psk(device: wrapless.Device):
    ...
    return psk_hash == Crypto.Hash.SHA256.SHA256Hash(PSK).digest()
...
def verify_otp_hash(otp):
    ...
    if received_hash == computed_hash:
        ...
        raise Exception(
            f"OTP hash incorrect: {received_hash} != {computed_hash}")
...
def fix_config_checksum(config):
    ...
    for short_idx in range(0, len(config) - 2, 2):
        short = int.from_bytes(config[short_idx:short_idx + 2],
                               byteorder="little")
        checksum += short
        checksum &= 0xFFFF
...
def replace_value_in_section(config, section_num, tag, value):
    ...
    for entry_base in range(section_base, section_base + section_size, 4):
        entry_tag = int.from_bytes(config[entry_base:entry_base + 2],
                                   byteorder="little")
        if entry_tag == tag:
...
def upload_config(device: wrapless.Device, calib_params: CalibrationParams):
    ...
    replace_value_in_section(chip_config, 2, DAC_L_TAG,
                             calib_params.dac_l << 4 | 8)
    replace_value_in_section(chip_config, 3, DAC_L_TAG,
                             calib_params.dac_l << 4 | 8)
    replace_value_in_section(chip_config, 2, DELTA_DOWN_TAG,
                             calib_params.delta_down << 8 | 0x80)
    fix_config_checksum(chip_config)

    device.upload_config(chip_config, 0.5)


def get_fdt_base_with_tx(device: wrapless.Device, tx_enable: bool,
...
def get_adjusted_dac(sensor_image: list[int], calib_image: list[int],
                     dac: int):
...
def validate_base_img(base_image_1: list[int], base_image_2: list[int],
                      image_threshold: int):
    ...
    logging.debug(
        f"Checking image data, avg: {avg:.2f}, threshold: {image_threshold}")
...
def update_all_base(device: wrapless.Device, calib_params: CalibrationParams):
    ...
    image_tx_enabled = get_image(device, True, True, "l", False, False,
                                 calib_params)

    fdt_data_tx_disabled = get_fdt_base_with_tx(device, False, calib_params)

    fdt_base_valid = is_fdt_base_valid(fdt_data_tx_enabled,
                                       fdt_data_tx_disabled,
                                       calib_params.delta_fdt)
    if not fdt_base_valid:
        raise Exception("Invalid FDT")

    image_tx_disabled = get_image(device, False, True, "l", False, False,
                                  calib_params)

    validate_base_img(image_tx_enabled, image_tx_disabled,
                      calib_params.delta_img)

    fdt_data_tx_enabled_2 = get_fdt_base_with_tx(device, True, calib_params)

    fdt_base_valid = is_fdt_base_valid(fdt_data_tx_enabled_2,
                                       fdt_data_tx_disabled,
                                       calib_params.delta_fdt)
    if not fdt_base_valid:
        raise Exception("Invalid FDT")
...
def generate_fdt_up_base(fdt_data, touch_flag,
                         calib_params: CalibrationParams):
    fdt_vals = []
    for idx in range(0, len(fdt_data), 2):
        fdt_val = int.from_bytes(fdt_data[idx:idx + 2], byteorder="little")
    ...
    for idx in range(0xC):
        if ((touch_flag >> idx) & 1) == 0:
            fdt_base_up_vals[idx] = (calib_params.delta_up * 0x100
                                     | calib_params.delta_up)
...
def wait_for_finger_down(device: wrapless.Device,
                         calib_params: CalibrationParams):
    fdt_data, touch_flag = device.wait_for_fdt_event(
        wrapless.FingerDetectionOperation.DOWN)
    calib_params.fdt_base_up = generate_fdt_up_base(fdt_data, touch_flag,
                                                    calib_params)
    return fdt_data


def wait_for_finger_up(device: wrapless.Device,
                       calib_params: CalibrationParams):
    fdt_data, _ = device.wait_for_fdt_event(
...
def main(product: int):
    ...
    fdt_base_valid = is_fdt_base_valid(event_fdt_data, manual_fdt_data,
                                       calib_params.delta_fdt)
    if fdt_base_valid:
    ...
    finger_image = get_image(device, True, True, "h", False, True,
                             calib_params)
    tool.write_pgm(finger_image, SENSOR_HEIGHT, SENSOR_WIDTH, "raw_finger.pgm")
