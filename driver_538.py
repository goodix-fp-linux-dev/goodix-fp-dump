...
def get_image(device: Device, tls_client: socket, tls_server: Popen) -> None:
    ...
    device.mcu_switch_to_fdt_mode(
        b"\x0d\x01\x28\x01\x22\x01\x28\x01\x24\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01"  # Last byte is for reply
    )
    ...
    data_file.write(tls_server.stdout.read(100))
    data_file.close()
